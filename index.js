...
// 3.0 Render the users in the DOM
function renderUsers(usersData) {
  console.log(usersData);
  const ul = document.getElementById("user-list-container");
  console.log(ul);

  // 3.1  Render a li tag for each of the users
  usersData.forEach((user, index) => {
    ...
    `;
    // 3.2 Apppend the current user li to the li tag
    ul.appendChild(li);
  });
}
...
function searchUsersByUsername() {
  ...
  //  loop throught all the users and render the one that match the search
